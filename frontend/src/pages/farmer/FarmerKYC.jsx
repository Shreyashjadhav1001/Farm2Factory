...
const FarmerKYC = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/api/farmer/kyc', formData, {
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
          ...
          <div className="pt-6">
            <button 
              type="submit" 
              disabled={loading}
              className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-300 text-white font-extrabold py-4 rounded-2xl shadow-xl shadow-emerald-500/20 transition-all flex items-center justify-center gap-3 active:scale-[0.98]"
            >
              {loading ? 'Processing...' : (
                <>
                  Submit KYC Data
